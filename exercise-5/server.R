...
shinyServer(function(input, output) {
  ...
  output$womensrole <- renderPlot({
    ...
    if(input$sex == "Men") {
      plot.df <- df %>% filter(sex == "Male")
    } else if (input$sex == "Women") {
      plot.df <- df %>% filter(sex == "Female")
    } else {
      plot.df <- df
    }
    ...
    if(input$thoughts == "Agree") {
      thoughts = plot.df$agree
    } else {
      thoughts = plot.df$disagree
    }
    ...
    ggplot(plot.df, aes(x=education, y=thoughts, color=factor(sex))) + geom_point()
    })
